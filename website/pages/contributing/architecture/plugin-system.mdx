...
RAT defines 6 plugin slots. Each slot has a specific interface (protobuf service definition) and responsibility:
...
| **permission** | `PermissionService` | Generic permission engine (path-based grants, groups, wildcards) | Noop (allow all) | acl-plugin |
...
The permission plugin is a generic, extensible authorization engine using path-based resource grants. It supersedes the basic `admin > write > read` model from the enforcement plugin with a fully declarative system where hierarchy is implicit in resource paths.
...
service PermissionService {
  ...
  // Access checks — direct grants + verb implications + wildcard path matching
  ...
  // Engine-managed groups (nestable, depth limit 5)
  ...
  // External group sync (Keycloak, Cognito, etc.)
  rpc SyncExternalGroups(SyncExternalGroupsRequest) returns (SyncExternalGroupsResponse);
}
```

**Community behavior**: No permission engine. Single-user platform — all access allowed.

**Pro behavior**: The ACL plugin implements `PermissionService` alongside the existing `SharingService` and `EnforcementService`. Key features:
...
- **Explicit wildcards**: `gold/*` grants access to all children, `*` is super admin — no surprise cascading
- **Global verb implication graph**: `admin` implies `write`, `read`, `execute`, `publish`, `delete` — resolved via BFS. `publish` implies `read` but not `write` (devs can edit, only release managers publish)
- **Extensible verbs**: `RegisterVerb` allows plugins to define custom verbs (e.g., `deploy`) with their own implications
- **Three principal types**: `USER` (direct grants), `GROUP` (engine-managed, nestable up to depth 5), `ROLE` (JWT groups from external providers)
- **Single SQL query**: `CheckAccess` resolves in one query — no recursive hierarchy walks
- **Batch access checks**: evaluate multiple resource/verb pairs in one call (used for list filtering)
- **Introspection**: "who has access to this pipeline?" and "what can this user access?"
- **Backward compatible**: existing `SharingService`/`EnforcementService` RPCs continue to work via dual-write wrappers

<Callout type="info">
The permission engine is additive — it coexists with the existing sharing and enforcement plugins. Existing clients that use `SharingService.ShareResource` or `EnforcementService.CanAccess` continue to work. New clients should prefer `PermissionService` for its richer feature set (groups, wildcards, verb implications).
</Callout>
...
| Permission engine | N/A | Path-based grants, verb implications, wildcards, groups |
