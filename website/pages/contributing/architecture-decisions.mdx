...
### Data Features and Permissions (v2.10 - v2.12)

| ADR | Title | Status | Summary |
|---|---|---|---|
| **013** | Landing Zones | Accepted | Standalone file drop areas for raw data uploads. Postgres-tracked metadata, multipart upload, DuckDB preview |
| **014** | Merge Strategies | Accepted | 6 write strategies (full_refresh, incremental, append_only, delete_insert, scd2, snapshot). Config merge with annotation overrides |
| **015** | Data Retention | Accepted | Configurable retention limits: runs per pipeline, .meta.yaml history, S3 log TTL. Background reaper cleanup |
| **016** | Portal Plugin UI | Accepted | Generic slot system for portal extensibility. `PluginSlot` component, `PluginRegistryProvider`, dynamic registry loading |
| **017** | Generic Permission Engine | Accepted | Zanzibar-inspired relation tuples. Action implication graphs (BFS), resource hierarchy inheritance, engine-managed groups with nesting, JWT/external role support. Backward-compatible with existing sharing/enforcement |
...
- **Working on permissions**: Read ADRs 010 and 017 to understand the ACL and permission engine evolution
